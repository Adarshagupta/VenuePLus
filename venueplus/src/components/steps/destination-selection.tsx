...
export function DestinationSelection({ tripData, onUpdate, onNext }: DestinationSelectionProps) {
  ...
  const handleDestinationSelect = (destination: Destination) => {
    console.log('Destination clicked:', destination.name) // Debug log
    onUpdate({ destination: destination.name })
    console.log('Navigating to next step...') // Debug log
    onNext()
  }
          ...
          {filteredDestinations.map((destination) => (
            <div
              ...
              onClick={(e) => {
                e.preventDefault()
                e.stopPropagation()
                console.log('Card clicked for:', destination.name)
                console.log('Card element:', e.currentTarget)
              ...
              title={`Click to select ${destination.name}`}
            >
              {/* Image Section */}
                ...
                <div className="absolute bottom-4 right-4">
                  <button 
                    onClick={(e) => {
                      e.preventDefault()
                      e.stopPropagation()
                      console.log('Heart clicked for:', destination.name)
                    }}
              ...
              <div className="p-6">
                ...
                <button
                  onClick={(e) => {
                    e.preventDefault()
                    e.stopPropagation()
                    console.log('Select button clicked for:', destination.name)
                    handleDestinationSelect(destination)
                  }}
